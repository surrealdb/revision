...
/// The name of the struct field decorator.
pub(crate) const ATTRIBUTE_NAME: &str = "revision";

/// Struct annotation constants.
pub(crate) const DEFAULT_FN: &str = "default_fn";
pub(crate) const CONVERT_FN: &str = "convert_fn";
pub(crate) const START_REVISION: &str = "start";
pub(crate) const END_REVISION: &str = "end";
...
pub fn revisioned(attrs: TokenStream, input: TokenStream) -> proc_macro::TokenStream {
	...
	let attrs = match Arguments::from_list(&attrs) {
		Ok(v) => v,
		Err(e) => {
			return TokenStream::from(Error::from(e).write_errors());
		}
	...
	// Decorate the struct definition
	(quote! {
		#[derive(revision::Revisioned)]
		#[revision(#revision)]
		#input
	})
	.into()
...
/// the `Revisioned` trait for structs and enums.
///
/// You should not use this derive macro directly, but instead should use the
/// procedural macro attribute #[revisioned] to decorate each struct or enum,
/// as that will automatically removed old fields from structs and enums.
///
/// This macro works by generating a single serializer implementation for the
/// latest revision of a struct, and multiple deserializer implementations for
/// each historical revision of a struct. There is no limit to the maximum
...
/// struct SomeStruct {
///     some_u32: u32,
///     #[revision(end = 2, convert_fn = "convert_some_u16")]
///     some_u16: u16,
///     #[revision(start = 2)]
///     some_u64: u64,
/// }
...
/// }
/// ```
#[proc_macro_derive(Revisioned, attributes(revision))]
pub fn impl_revisioned(input: TokenStream) -> proc_macro::TokenStream {
	// Parse the derive macro
