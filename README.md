...
- **`skip`** (disabled by default): Enables `SkipRevisioned` / `SkipCheckRevisioned`, `SkipRevisioned::skip_revisioned_slice` (used by `skip_slice` for zero-copy bulk skipping over borrowed bytes where supported), aliases `skip_reader` / `skip_check_reader`, `skip_revisioned`, `skip_slice`, `skip_check_revisioned`, `skip_check_slice`, and derive output for skipping encoded values (`#[revisioned(..., skip = false)]` to opt out per type). Emitted impls sit behind `cfg(feature = "skip")`; enable the feature on the **`revision`** dependency and expose a matching **`skip`** feature in your crate when you rely on procedural output.
