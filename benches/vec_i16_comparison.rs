...
struct BincodeVecI16(Vec<i16>);

// Generate test data for benchmarking
fn generate_test_data(size: usize) -> Vec<i16> {
	(0..size).map(|i| (i as i16).wrapping_sub(size as i16 / 2)).collect()
}

fn benchmark_serialization(c: &mut Criterion) {
	let sizes = [100, 10_000, 1_000_000];

	let mut group = c.benchmark_group("Vec<i16> Serialization");

	for &size in &sizes {
		let data = generate_test_data(size);
		// i16 is 2 bytes per element
		group.throughput(Throughput::Bytes((size * 2) as u64));

		// Benchmark revision Vec<i16> serialization
		...
		let bincode_data = BincodeVecI16(data.clone());
...
fn benchmark_deserialization(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("Vec<i16> Deserialization");
	...
	for &size in &sizes {
		...
		// Benchmark revision Vec<i16> deserialization
		group.bench_with_input(BenchmarkId::new("Revision", size), &size, |b, _| {
			b.iter(|| {
				let deserialized: Vec<i16> =
		...
		group.bench_with_input(BenchmarkId::new("Bincode", size), &size, |b, _| {
			b.iter(|| {
				let deserialized: BincodeVecI16 =
