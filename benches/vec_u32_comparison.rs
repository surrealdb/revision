...
fn benchmark_serialization(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("Vec<u32> Serialization");

	for &size in &sizes {
		let data = generate_test_data(size);
		// u32 is 4 bytes per element
		group.throughput(Throughput::Bytes((size * 4) as u64));

		// Benchmark revision Vec<u32> serialization
		...
		let bincode_data = BincodeVecU32(data.clone());
...
fn benchmark_deserialization(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("Vec<u32> Deserialization");
	...
	for &size in &sizes {
		...
		// Benchmark revision Vec<u32> deserialization
		group.bench_with_input(BenchmarkId::new("Revision", size), &size, |b, _| {
			b.iter(|| {
		...
		group.bench_with_input(BenchmarkId::new("Bincode", size), &size, |b, _| {
			b.iter(|| {
				let deserialized: BincodeVecU32 =
