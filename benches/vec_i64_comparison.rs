...
fn benchmark_serialization(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("Vec<i64> Serialization");
	...
	for &size in &sizes {
		...
		// Benchmark revision Vec<i64> serialization
		...
		let bincode_data = BincodeVecI64(data.clone());
...
fn benchmark_deserialization(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("Vec<i64> Deserialization");
	...
	for &size in &sizes {
		...
		// Benchmark revision Vec<i64> deserialization
		group.bench_with_input(BenchmarkId::new("Revision", size), &size, |b, _| {
			b.iter(|| {
				let deserialized: Vec<i64> =
		...
		group.bench_with_input(BenchmarkId::new("Bincode", size), &size, |b, _| {
			b.iter(|| {
				let deserialized: BincodeVecI64 =
