...
impl DeserializeRevisioned for RevisionSpecialisedVecU8 {
	...
	fn deserialize_revisioned<R: Read>(reader: &mut R) -> Result<Self, Error> {
		...
		unsafe {
			// Get a mutable slice of the uninitialized memory
			let uninit_slice = std::slice::from_raw_parts_mut(vec.as_mut_ptr(), len);
			...
			// 3. u8 doesn't require initialization (it's Copy and has no Drop)
